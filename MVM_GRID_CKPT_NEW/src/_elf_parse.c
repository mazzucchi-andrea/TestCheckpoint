...
void build_patches(void) {
...
#if CKPT
    uint64_t ckpt_byte_code = (uint64_t)ckpt_byte_assembly;
    uint64_t ckpt_word_code = (uint64_t)ckpt_word_assembly;
    uint64_t ckpt_dword_code = (uint64_t)ckpt_dword_assembly;
    uint64_t ckpt_code = (uint64_t)ckpt_assembly;

    size_t ckpt_byte_code_size =
        (uintptr_t)ckpt_word_assembly - (uintptr_t)ckpt_byte_assembly;
    size_t ckpt_word_code_size =
        (uintptr_t)ckpt_dword_assembly - (uintptr_t)ckpt_word_assembly;
    size_t ckpt_dword_code_size =
        (uintptr_t)ckpt_assembly - (uintptr_t)ckpt_dword_assembly;
    ...
    for (i = 0; i < target_instructions; i++) {
        ...
        if (instructions[i].data_size == 1) {
            memcpy((char *)(patches[i].code), (char *)(ckpt_byte_code),
                   ckpt_byte_code_size);
            patches[i].code = patches[i].code + ckpt_byte_code_size;
        } else if (instructions[i].data_size == 2) {
            memcpy((char *)(patches[i].code), (char *)(ckpt_word_code),
                   ckpt_word_code_size);
            patches[i].code = patches[i].code + ckpt_word_code_size;
         } else if (instructions[i].data_size == 4) {
            memcpy((char *)(patches[i].code), (char *)(ckpt_dword_code),
                   ckpt_dword_code_size);
            patches[i].code = patches[i].code + ckpt_dword_code_size;
        } else {
            memcpy((char *)(patches[i].code), (char *)(ckpt_code),
                   ckpt_code_size);
        ...
        if (instructions[i].data_size == 1) {
            patches[i].code =
                patches[i].code - save_reg_lea - ckpt_byte_code_size;
        } else if (instructions[i].data_size == 2) {
            patches[i].code =
                patches[i].code - save_reg_lea - ckpt_word_code_size;
        } else if (instructions[i].data_size == 4) {
            patches[i].code =
                patches[i].code - save_reg_lea - ckpt_dword_code_size;
        } else {
